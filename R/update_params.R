...
update_cluster_coefs <- function(model_params, hyperpars){
  B0 <- model_params$B0
  clusters <- model_params$clusters
  Z <- model_params$Z
  cluster_coefs <- model_params$cluster_coefs
  ...
  for (k in 1:K){
    ...
    if (length(cluster_indices) == 0){
      cluster_coefs[, , k] <- B0 + rmvnorm(q, rep(0, p), Omega)
    } else{
      Z_cluster <- Z[cluster_indices, , drop = F]
      X_cluster <- X[cluster_indices, , drop = F]
      cov <- cluster_covs[, , k]
      prec <- cluster_precs[, , k]
      for (j in 1:p){
        Sigma12 <- cov[j, -j, drop = F]
        Sigma22_inv <- solve(cov[-j, -j, drop = F])
        sigma_j_tilde <- (cov[j, j] - Sigma12 %*% Sigma22_inv %*% t(Sigma12))[1, 1]
        z_tilde <- Z_cluster[, j, drop = F] - t(Sigma12 %*% Sigma22_inv %*%
          t(Z_cluster[, -j, drop = F] - X_cluster %*% cluster_coefs[, -j, k, drop = F]))
        var_kj <- solve(tau[j] * diag(q) + t(X_cluster) %*% X_cluster / sigma_j_tilde)
        mean_kj <- var_kj %*% (tau[j] * B0[, j, drop = F] + t(X_cluster) %*% z_tilde /
                                 sigma_j_tilde)
        cluster_coefs[, j , k] <- rmvnorm(q, mean = mean_kj, sigma = var_kj)
      }
    }
  }
  cluster_coefs
  model_params$cluster_coefs <- cluster_coefs
  model_params
}
...
update_cluster_covs <- function(model_params, hyperpars){
   ...
   for (k in 1:K){
     ...
     if (length(cluster_indices) == 0){
       ...
       Z_cluster <- Z[cluster_indices, , drop = F]
       X_cluster <- X[cluster_indices, , drop = F]
       M <- X_cluster %*% cluster_coefs[, , k]
       Zmm <- Z_cluster - M
       S <- matrix(0, nrow = p, ncol = p)
       for (i in 1:n_k){
         S = S + t(Zmm[i, , drop = F]) %*% Zmm[i, , drop = F]
       }
       cluster_covs[, , k] <- riwish(nu + n_k, Sigma + S)
       cluster_precs[, , k] <- solve(cluster_covs[, , k])
     }
   }
...
update_zobs <- function(model_params, hyperpars, transformations){
  Y <- model_params$Y
  E <- is.na(Y)
  Z <- model_params$Z
  X <- model_params$X
  ...
  K <- dim(cluster_coefs)[3]
  for (k in 1:K){
    coefs <- matrix(cluster_coefs[, , k], ncol = p)
    cov <- cluster_covs[, , k]
    prec <- cluster_precs[, , k]
    for (j in 1:p){
      ...
      X_obs_disc <- X_obs[obs_discrete, , drop = F]
