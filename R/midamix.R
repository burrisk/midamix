midamix <- function(data, imputations = 10, max_clusters = 15, n_iter = 1000,
                    burnin = 100, validator = NULL, transformations = NULL, seed = NA){
  ...
  for (j in 1:p){
    ...
    Z[yj_obs, j] <- truncnorm::rtruncnorm(length(yj_obs), a = boundsZ_obs[,1], b = boundsZ_obs[,2])
  ...
  imp_tibbles <- lapply(imp_list, function(df){
    df <- df %>%
      dplyr::as_tibble() %>%
      dplyr::mutate_at(which(integers), as.integer) %>%
